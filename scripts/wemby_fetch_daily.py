...
def fetch_game_log(last_n_games: int = 10):
    """
    Récupère les N derniers matchs de Wembanyama.
    Par défaut, récupère les 10 derniers matchs pour optimiser.
    Utilise last_n_games=0 pour récupérer tous les matchs (premier run).
    """
    from nba_api.stats.endpoints import playergamelog
    import time
    
    log(f"Fetch PlayerGameLog pour player_id={PLAYER_ID}, season={CURRENT_SEASON}, last_n_games={last_n_games}")
    
    # Si last_n_games=0, on ne passe pas le paramètre (récupère tout)
    if last_n_games > 0:
    ...
    # Si on veut limiter aux N derniers
    if last_n_games > 0 and len(df) > last_n_games:
...
def count_games_in_db() -> int:
    """Compte le nombre de matchs en base."""
...
def main():
    ...
    try:
        # 2. Compter les matchs en base pour savoir si c'est le premier run
        games_in_db = count_games_in_db()
        is_first_run = games_in_db == 0
        
        if is_first_run:
            log("🚀 Premier run détecté - récupération de tous les matchs de la saison")
            last_n_games = 0  # Tous les matchs
        else:
            log(f"📊 {games_in_db} matchs en base - récupération des 5 derniers matchs uniquement")
            last_n_games = 5  # Seulement les 5 derniers pour optimiser
        
        # 3. Fetch le game log
        game_log_df = fetch_game_log(last_n_games=last_n_games)
        ...
        # 4. Récupérer les matchs déjà en base
        existing_ids = get_existing_game_ids()
        
        # 5. Traiter uniquement les nouveaux matchs
        new_games_count = 0
